...
"oui_player_myspace":https://github.com/NicolasGraph/oui_player_myspace is a "oui_player":https://github.com/NicolasGraph/oui_player v2+ extension to easily embed "MySpace":https://myspace.go.com/ provided videos in "Textpattern":https://textpattern.com/ powered websites.
...
oui_player_myspace's minimum requirements:
...
# "Download":https://github.com/NicolasGraph/oui_player_myspace/releases the compiled plugin file or the source to compile a customized file.
...
# Install oui_player_myspace:
@$ composer require oui/oui_player_myspace@
...
bc. $ composer update oui/oui_player_myspace
...
bc. $ composer remove oui/oui_player_myspace

h2. Tags and parameters

h3. oui_myspace vs oui_player

While the @<txp:oui_player />@ tag is able to embed any video from any extension-related provider, the @<txp:oui_myspace />@ tag is a shortcut to embed MySpace videos only. It is strictly equivalent to @<txp:oui_player provider="myspace" />@.
...
* <strong>Name of a field</strong> containing any of the above value.
...
h3. oui_if_myspace vs oui_if_player

p. While the @<txp:oui_if_player>…</txp:oui_if_player>@ tag is able to check an URL against any URL from any extension-related provider, the @<txp:oui_if_myspace>…</txp:oui_if_myspace>@ tag is a shortcut to check an URL against MySpace related URL schemes only. It is strictly equivalent to @<txp:oui_if_player provider="myspace">…</txp:oui_if_player>@.
...
"oui_player":https://github.com/NicolasGraph/oui_player preference related field value: @https://myspace.com/myspace/video/ok-go-the-ferris-wheel-interviews/109583400@.

h3. Embed a player by defining its provider and a media ID through attributes

bc(language-markup). <txp:oui_player provider="myspace" play="109583400" />

…or:

bc(language-markup). <txp:oui_myspace play="109583400" />
...
oui_player_myspace version 1.0.0, Copyright (C) 2018 Nicolas Morand
...
* *oui_player_myspace v1.0.0-beta* (2018-06-24)
